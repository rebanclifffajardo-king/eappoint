...
<input type="hidden" id="setpassword" value="<?php echo $admin->password; ?>"/>
<div class="wrapper">
 <!-- /.navbar -->
...
">
      ...
      <div class="container-fluid">
        <div class="row">
          ...
          <div class="col-md-12">
            ...
            <li class="nav-item"><a  id="nav-type" class="nav-link active"  href="#type_tab" data-toggle="tab" >Clinic Types </a></li>
            <li class="nav-item"><a  id="nav-personal" class="nav-link "  href="#personal_tab" data-toggle="tab" >Personal Details </a></li>
            ...
            <div class=" active tab-pane" id="type_tab">
            <hr/>
             
                      <a class="btn btn-success btn-sm" href="#" data-toggle="modal" data-target="#addtypemodal" > <i class="fas fa-plus"></i>&nbsp; Add Type </a>
                      <br/><br/>
                      <img id="type_load" src="../images/ajax-loader-big.gif" style="  display: block;margin-left: auto;margin-right: auto;">

                         <table id="typetbl" class="table table-condensed" width="100%">
                        <thead>
                        <tr>
            ...
            <div class=" tab-pane" id="personal_tab">
            <hr/>
            ...
            <div class="row">
                <div class="col-sm-6">
                    ...
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control" id="name" placeholder="John" value="<?php echo $admin->name; ?>">
                    </div>
          ...
          <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
              <div class="modal-header bg-success">
                <h4 class="modal-title">Add Clinic Type</h4>
              ...
              <div class="modal-body">
                <form role="form">
                    ...
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control" id="typename" placeholder="ex. dental">
                    </div>
                    <div class="form-group">
                      <label for="roomname">Room</label>
                      <input type="text" class="form-control" id="roomname" placeholder="ex. room201">
                    </div>
              ...
              <div class="modal-footer justify-content-between">
                ...
                <button type="button" id="savetypebtn" class="btn btn-success">Save changes</button>
              </div>
            </div>
            <!-- /.modal-content -->
          </div>
  ...
  <div class="modal fade" id="edittypemodal">
          <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
              <div class="modal-header bg-success">
                <h4 class="modal-title">Edit Clinic Type</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>
              <div class="modal-body">
                <form role="form">
                <input type="hidden"  id="e_typeid"  >
                    <div class="form-group">
                      <label for="e_typename">Type Name</label>
                   
                      <input type="text" class="form-control" id="e_typename" placeholder="ex. dental">
                    </div>
                    <div class="form-group">
                      <label for="e_roomname">Room</label>
                      <input type="text" class="form-control" id="e_roomname" placeholder="ex. room201">
                    </div>
              ...
              <div class="modal-footer justify-content-between">
                <button type="button"  class="btn btn-danger" data-dismiss="modal">Close</button>
                <button type="button" id="updatetypebtn" class="btn btn-success">Update changes</button>
              </div>
            </div>
            <!-- /.modal-content -->
          </div>
       ...
       success:  function(response){
          ...
          var col="";
            ...
            var room = response[i].room;
              ...
              col+='<td> '+room+' </td>';
              col+='<td>';
              col+='<button  id="edittypebtn" data-id="'+type_id+'" data-type_name="'+type_name+'" data-room="'+room+'" class="btn btn-success btn-sm" data-toggle="modal" data-target="#edittypemodal"   href="#">  <i class="fas fa-pencil-alt"></i>&nbsp; Edit </button>';
              ...
              col+='<button type="button" id="edittypebtn" data-id="'+type_id+'" data-type_name="'+type_name+'" data-room="'+room+'" class="dropdown-item" data-toggle="modal" data-target="#edittypemodal"  class="dropdown-item" href="#">  <i class="fas fa-pencil-alt"></i>&nbsp; Edit </button>';
              col+='<button type="button" id="deletetypebtn"  data-id="'+type_id+'" class="dropdown-item">  <i class="fas fa-window-close"></i>&nbsp; Delete </button>';
              col+='  </div>';
              col+='  </button>';
              col+='  </div>';
*/
        ...
        $("#typetbl tbody").html(col);

            initDatatable("#typetbl");
...
$(document).on('click', '#updatebtn', function () {
  ...
  var name= $("#name").val();
    ...
    if(name==""){
       showAlert('error','Empty Fields','Please select enter name! ');
       return;
     }
...
$.ajax({
  ...
  success: function(response){
    ...
    var error = response[0].error;
  // alert(status);
    if(status=="success")  showAlert('success','Success','Type added!');
    if(status=="exist")  showAlert('error','Error',error);
    if(status=="error")  showAlert('error','Error',error);
...
$.ajax({
  ...
  success: function(response){
    ...
    var error = response[0].error;
   //alert(status);
    if(status=="success")  showAlert('success','Success','Type updated!');
    if(status=="exist")  showAlert('error','Error',error);
    if(status=="error")  showAlert('error','Error',error);
...
function dialog(icon, title, text, values, type ){
  ...
  if (result.isConfirmed) {
    ...
    if(type == "insert")  insertType(values);
    if(type == "update")  updateType(values);
...
$(document).on('click', '#edittypebtn', function () {
  $("#e_typename").val($(this).data('type_name'));
  $("#e_roomname").val($(this).data('room'));
  $("#e_typeid").val($(this).data('id'));
...
$(document).on('click', '#savetypebtn', function () {
  ...
  //error
    if(typename==""){
      showAlert('error','Empty Fields','Please add type name! ');
      return;

    }
    if(roomname==""){
      showAlert('error','Empty Fields','Please add room name! ');
      return;
    ...
    dialog("question",
      ...
      "This will add new entry in the database",
      values,
      "insert" );

});

$(document).on('click', '#updatetypebtn', function () {
  var type_id= $("#e_typeid").val();
  var typename= $("#e_typename").val();
  var roomname= $("#e_roomname").val();
