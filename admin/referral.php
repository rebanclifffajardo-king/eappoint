      ...
      <div class="container-fluid">
        <div class="row">
          <div class="col-lg-12">
            ...
            <li class="nav-item"><a  id="nav-receive" class="nav-link active"  href="#receive_tab" data-toggle="tab" >Received </a></li>
            <li class="nav-item"><a  id="nav-sent" class="nav-link "  href="#sent_tab" data-toggle="tab" >Sent </a></li>
           
            </ul>
            <div class="tab-content">

            <div class=" active tab-pane" id="receive_tab">
            <hr/>
            <a class="btn btn-success" href="#" data-toggle="modal" data-target="#addModal" > <i class="fas fa-plus"></i>&nbsp; Add Referral </a>
                <br/><br/>
               
                      <img id="table_load" src="../images/ajax-loader-big.gif" style="  display: block;margin-left: auto;margin-right: auto;">

                         <table id="referraltbl" class="table table-hover" width="100%">
                        <thead>
                        <tr>
                          <th>Status</th>
                          <th>From</th>
            ...
            <hr/>
                      ...
                      <img id="table1_load" src="../images/ajax-loader-big.gif" style="  display: block;margin-left: auto;margin-right: auto;">

                         <table id="senttbl" class="table table-hover" width="100%">
                        <thead>
                        <tr>
                          <th>Status</th>
                          <th>To</th>
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              <form role="form">
                      ...
                      <select  id="patienttype" class="custom-select select2">
                      <option value=""></option>
                      <?php 
...
function approveRequest(values){
  $.ajax({
    ...
    success: function(response){
      ...
      if(status=="success")  showAlert('success','Success','Referral Approved');
  ...
  $.ajax({
    ...
    success: function(response){
     ...
     // alert(status);
      ...
      if(status=="success")  showAlert('success','Success','Referral Denied');
      if(status=="exist")  showAlert('error','Error',error);
      if(status=="error")  showAlert('error','Error',error);
  ...
  $.ajax({
       ...
       success:  function(response){
          ...
          var col="";
            ...
            var file_attached = response[i].file_attached;
            var date_sent = response[i].date_sent;
            var type= response[i].type;
            var sender= response[i].sender;
            var status= response[i].status;
            var clinic_name= response[i].clinic_name;
            ...
            var longitude= response[i].longitude; 
              ...
              col+='<td> '+clinic_name+' </td>';
              col+='<td> '+date_sent+' </td>';
              col+='<td> '+name+' </td>';
              col+='<td> '+message+' </td>';
              col+='<td> <a href="'+file_attached+'" download> Download </a></td>';
              ...
              col+='<button type="button"  data-id='+id+' id="btnApprove" class="dropdown-item"   >  Approve &nbsp; <i class="fas fa-check"></i> </button>';
              col+='<button type="button" id="btnDeny" data-id='+id+' class="dropdown-item" class="dropdown-item"  >   Disapproved &nbsp; <i class="fas fa-times"></i></button>';
              col+='  </div>';
              col+='  </button>';
              col+='  </div>';
              col+='  </td>';
        ...
        $("#referraltbl tbody").append(col);

            initDatatable("#referraltbl");
...
function getSentReferral(){
  restartDatatable("#senttbl");
...
var data = {
  ...
  $.ajax({
       ...
       success:  function(response){
        ...
        $("#senttbl tbody").append(col);

            initDatatable("#senttbl");
...
$(document).on('click', '#btnDeny', function () {
  ...
  dialog("question",
  "Are you sure you want to deny this referral?",
  "This will not undo any changes in the database",
  values,
  "denied" );
