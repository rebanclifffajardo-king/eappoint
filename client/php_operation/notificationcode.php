...
$display=mysqli_real_escape_string($conn,$_POST['display']);
 ...
 if($display=="user") $response = getNotifUser();
...
function deleteNotifCount(){
    ...
    mysqli_query($conn,"DELETE FROM countnotiftbl_user WHERE user_id='$client_id'");
  ...
  $client_id = $_SESSION['client_id'];
   ...
   $date=date('Y-m-d');
    ...
    $response=array();
     ...
     $sql = "SELECT * FROM notificationtbl_user WHERE user_id='$client_id' ORDER BY id DESC LIMIT 4";
  ...
  $client_id = $_SESSION['client_id'];
   ...
   $date=date('Y-m-d');
    ...
    $response=array();
     ...
     $response[]=array(
      "countnotif"=>(countnotif($client_id))
    );
 	...
 	function countnotif($client_id){
		global $conn;
	   $sql=mysqli_query($conn,"SELECT * FROM countnotiftbl_user WHERE user_id='$client_id'");
