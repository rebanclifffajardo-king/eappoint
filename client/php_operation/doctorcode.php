...
$display=mysqli_real_escape_string($conn,$_POST['display']);
 ...
 if($display=="all") $response = getDoctorlist(); 
 ...
 if($display=="doctor") $response = getDoctorDetails();
...
return $response;
 ...
 function updateSchedDay($time,$max, $start, $end, $day_, $doctor_id){
  ...
  $sql = "UPDATE schedtbl SET consultation_time='$time', max_client='$max'  $consult WHERE doctor_id='$doctor_id' AND day_='$day_' ";
  $query = mysqli_query($conn,$sql);
 ...
 function getDoctorDetails(){
  ...
  //$id="1";
   ...
   INNER JOIN schedtbl s ON d.doctor_id=s.doctor_id WHERE d.doctor_id='$id' ";
   ...
   while($row = mysqli_fetch_assoc($query)){
     ...
     $clinic_info = class_getClinicData($clinictype);
